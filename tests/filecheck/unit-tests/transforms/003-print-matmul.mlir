...
builtin.module {
  ...
  builtin.module attributes {kernel_type = "compute"} {
    func.func @MAIN() {
      ...
      "comp.mm_init"(%2, %3, %4, %0) : (i32, i32, i32, i32) -> ()
      "cb.wait_front"(%3, %1) : (i32, i32) -> ()
      "cb.wait_front"(%2, %1) : (i32, i32) -> ()
      "comp.tile_regs_acquire"() : () -> ()
      "comp.matmul_tiles"(%2, %3, %0, %0, %0, %0) : (i32, i32, i32, i32, i32, i32) -> ()
...
// CHECK: extern "C" void host_entry(float* fn_arg_0, float* fn_arg_1, float* fn_arg_2) {
...
// CHECK: KernelHandle writer_kernel = CreateKernel(prog, "writer.cpp", CoreCoord{0, 0}, DataMovementConfig{.processor = DataMovementProcessor::RISCV_1, .noc=NOC::RISCV_1_default});
// CHECK: KernelHandle compute_kernel = CreateKernel(prog, "compute.cpp", CoreCoord{0, 0}, ComputeConfig {.math_fidelity = MathFidelity::LoFi, .fp32_dest_acc_en = false, .math_approx_mode = false, .compile_args = {}});
// CHECK: void MAIN {
// CHECK: binary_op_init_common(0, 1, 2);
// CHECK: mm_init(0, 1, 2, 0);
// CHECK: cb_wait_front(1, 1);
// CHECK: cb_wait_front(0, 1);
// CHECK: tile_regs_acquire();
// CHECK: matmul_tiles(0, 1, 0, 0, 0, 0);
